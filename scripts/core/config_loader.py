...
def get_engine(name: str, path: Optional[str] = None):
    ...
    for category in ["search_engines", "news_sources", "academic_sources", "wechat_sources", "social_sources", "special_engines"]:
...
def list_source_categories(path: Optional[str] = None) -> dict[str, int]:
    config = _load_config(path)
    categories = ["search_engines", "news_sources", "academic_sources", "wechat_sources", "social_sources", "special_engines"]
