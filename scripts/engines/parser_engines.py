"""Parser-based search engines that extract structured results from HTML."""
...
class DuckDuckGoEngine(SearchEngine):
    """DuckDuckGo search engine using HTML scraping (no API key required)."""
    ...
    def build_url(self, query: str, **kwargs) -> str:
        ...
        if "kp" in kwargs:
            params["kp"] = str(kwargs["kp"])
        if "kl" in kwargs:
            params["kl"] = str(kwargs["kl"])
